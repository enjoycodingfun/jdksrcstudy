 ...
 *下面两个方法可以关闭线程池shutdown和shutdownNow，区别在于前者允许之前提交的任务在关闭之前执行
 * 后者会尝试直接关闭，不允许执行等待队列中的任务，也不允许提交新的任务
 ...
 *submit方法会返回Future对象，可以用来取消任务执行或者等待任务执行完毕
...
public interface ExecutorService extends Executor {
     ...
     * 一旦某个任务正常执行完毕或者异常返回了，剩下未完成的任务会被取消
