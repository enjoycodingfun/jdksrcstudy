...
public class ThreadPoolExecutor extends AbstractExecutorService {
     ...
     *
     * RUNNING -> SHUTDOWN
     * 当调用了 shutdown() 后，会发生这个状态转换，在finalize()方法中隐含着
     *    On invocation of shutdown(), perhaps implicitly in finalize()
     * (RUNNING or SHUTDOWN) -> STOP
     * 当调用 shutdownNow() 后，会发生这个状态转换，这下要清楚 shutDown() 和 shutDownNow() 的区别了
     *    On invocation of shutdownNow()
     * SHUTDOWN -> TIDYING
     * 当任务队列和线程池都清空后，会由 SHUTDOWN 转换为 TIDYING
     *    When both queue and pool are empty
     * STOP -> TIDYING
     * 当任务队列清空后，发生这个转换
     *    When pool is empty
     * TIDYING -> TERMINATED
     * 这个前面说了，当 terminated() 方法结束后
     ...
     * below).
     */
    //线程池的运行状态 (runState) 和线程池内有效线程的数量 (workerCount)，高3位保存runState，低29位保存workerCount(五亿多)，两个变量之间互不干扰
        ...
        try {
            ...
            if (t != null) {
                ...
                try {
                    ...
                    //重新获取线程池状态，防止在获得锁之前线程池被关闭或者使用线程工厂创建线程失败的情况
                    int rs = runStateOf(ctl.get());
...
//    - 线程池处于 STOP，不仅不接受新的线程，连 workQueue 中的线程也不再执行
    private Runnable getTask() {
        ...
        for (;;) {
            ...
            //如果当前线程数大于最大线程数或者允许超时的情况下已经超时而且当前线程数大于1或者当前队列为空
            ...
            try {
                // 到 workQueue 中获取任务，根据是否允许超时调用非阻塞的poll或者阻塞的take
                Runnable r = timed ?
...
// 把队列队头的任务(也就是等待了最长时间的)直接扔掉，然后提交当前任务到等待队列中
